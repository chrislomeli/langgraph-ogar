...
from ogar.runtime.sidecars.instrumented_graph import Middleware
from ogar.adapters.tools.envelope import ToolResultEnvelope
...
class StateMediator(Middleware):
    ...
    def _extract_envelope(result: Any) -> ToolResultEnvelope | None:
        ...
        if isinstance(result, dict) and "_meta" in result:
            try:
                from ogar.adapters.tools.envelope import ToolResultMeta
