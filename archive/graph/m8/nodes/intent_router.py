"""
Intent Router — M8 adds refinement intent routing.

When the user says "refine" and there's already a plan in state,
we route to the refinement subgraph. The router also snapshots
the current plan/compile_result into previous_* fields so the
refinement subgraph can do scoped recompilation.
"""
...
def intent_router(state: ParentState) -> dict:
    ...
    if any(kw in msg for kw in ["save", "store"]):
        ...
        # Snapshot current state for refinement
...
def _extract_title(msg: str, verb: str) -> str:
    """Extract project title from messages like 'save as My Rock Tune'."""
