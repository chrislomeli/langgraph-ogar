"""
Async graph database write operations.

Async version of writer.py using neo4j AsyncGraphDatabase.
...
class AsyncGraphMusicWriter:
    """
    Async version of GraphMusicWriter.
    ...
    Usage:
        async with AsyncGraphMusicWriter() as writer:
            cid = await writer.create_composition("My Song")
            cvid = await writer.commit_composition_version(cid, spec)
    """
    ...
    async def __aenter__(self) -> "AsyncGraphMusicWriter":
        return self
    ...
    async def _execute_void(self, query: str, params: dict) -> None:
    ...
    async def create_section(self, name: str, *, sid: Optional[str] = None) -> str:
        """Create Section identity node."""
        sid = sid or _uuid()
        rows = await self._execute(CQL_CREATE_SECTION, {
            "sid": sid,
            "name": name,
            "created_at": _now_iso(),
        })
    ...
    async def create_composition(self, title: str, *, cid: Optional[str] = None) -> str:
        """Create Composition identity node."""
        cid = cid or _uuid()
        rows = await self._execute(CQL_CREATE_COMPOSITION, {
            "cid": cid,
            "title": title,
            "created_at": _now_iso(),
        })
    ...
    async def create_track(self, name: str, *, tid: Optional[str] = None) -> str:
        """Create Track identity node."""
        tid = tid or _uuid()
        rows = await self._execute(CQL_CREATE_TRACK, {
            "tid": tid,
            "name": name,
            "created_at": _now_iso(),
        })
    ...
    async def commit_meter_map(
        ...
        *,
        ...
        rows = await self._execute(CQL_CREATE_METERMAP_VERSION, {
            "content_hash": h,
            "meter_vid": meter_vid,
            "created_at": _now_iso(),
        ...
        for i, change in enumerate(meter_map.changes):
            await self._execute_void(CQL_LINK_METERMAP_CHANGE, {
                "meter_vid": meter_vid,
    ...
    async def commit_tempo_map(
        ...
        *,
        ...
        rows = await self._execute(CQL_CREATE_TEMPOMAP_VERSION, {
            "content_hash": h,
            "tempo_vid": tempo_vid,
            "created_at": _now_iso(),
            "schema_version": schema_version,
        ...
        for i, change in enumerate(tempo_map.changes):
            await self._execute_void(CQL_LINK_TEMPOMAP_CHANGE, {
                "tempo_vid": tempo_vid,
                "at_bar": change.at_bar,
    ...
    async def _commit_pitch_by_props(self, props: dict) -> str:
        """Helper: commit pitch from pre-computed properties."""
        rows = await self._execute(CQL_CREATE_PITCH, {
    ...
    async def commit_event(self, event, *, schema_version: int = 1) -> str:
        ...
        rows = await self._execute(CQL_CREATE_EVENT_VERSION, {
            "content_hash": h,
            "evid": evid,
            "created_at": _now_iso(),
            "schema_version": schema_version,
        ...
        for rel_type, target_props, rel_props in rels:
            if rel_type == "HAS_PITCH":
                ...
                await self._commit_pitch_by_props(target_props)
                await self._execute_void(CQL_LINK_EVENT_PITCH, {
                    "evid": evid,
                    "pitch_hash": pitch_hash,
                    "i": rel_props["i"],
                })
            elif rel_type == "HAS_ARTICULATION":
                await self._execute_void(CQL_LINK_EVENT_ARTICULATION, {
                    "evid": evid,
                    "artic_name": target_props["name"],
                    "i": rel_props["i"],
                })
            elif rel_type == "HAS_LYRIC":
                await self._execute_void(CQL_LINK_EVENT_LYRIC, {
                    "evid": evid,
    ...
    async def commit_measure(
        ...
        *,
        ...
        rows = await self._execute(CQL_CREATE_MEASURE_VERSION, {
            "content_hash": h,
            "mvid": mvid,
            "created_at": _now_iso(),
            "schema_version": schema_version,
        ...
        for i, event in enumerate(measure.events):
            evid = await self.commit_event(event, schema_version=schema_version)
            await self._execute_void(CQL_LINK_MEASURE_EVENT, {
                "mvid": mvid,
                "evid": evid,
                "i": i,
            })
    ...
    async def commit_section_version(
        self,
        sid: str,
        spec: SectionSpec,
        *,
        ...
        rows = await self._execute(CQL_CREATE_SECTION_VERSION, {
            "content_hash": h,
            "svid": svid,
            "sid": sid,
            "created_at": _now_iso(),
        ...
        for i, measure in enumerate(spec.measures):
            mvid = await self.commit_measure(measure, schema_version=schema_version)
            await self._execute_void(CQL_LINK_SECTION_MEASURE, {
                "svid": svid,
                "mvid": mvid,
                "i": i,
            })
        
        await self._execute_void(
            CQL_UPDATE_LATEST % {
                "IDENT_LABEL": "Section",
                "IDENT_KEY": "sid",
    ...
    async def commit_track_version(
        self,
        tid: str,
        spec: TrackSpec,
        *,
        ...
        rows = await self._execute(CQL_CREATE_TRACK_VERSION, {
            "content_hash": h,
            "tvid": tvid,
            "tid": tid,
            "created_at": _now_iso(),
        ...
        for ordinal, placement in enumerate(spec.placements):
            await self._execute_void(CQL_LINK_TRACK_PLACEMENT, {
                "tvid": tvid,
    ...
    async def commit_composition_version(
        self,
        cid: str,
        ...
        *,
        ...
        meter_vid = await self.commit_meter_map(spec.meter_map, schema_version=schema_version)
        tempo_vid = await self.commit_tempo_map(spec.tempo_map, schema_version=schema_version)
        
        props = CompositionAdapter.to_properties(spec)
        h = CompositionAdapter.compute_hash(spec)
        cvid = _uuid()
        
        rows = await self._execute(CQL_CREATE_COMPOSITION_VERSION, {
            "content_hash": h,
            "cvid": cvid,
            "cid": cid,
            "created_at": _now_iso(),
        ...
        await self._execute_void(CQL_LINK_COMPOSITION_TIMELINE, {
            "cvid": cvid,
            "meter_vid": meter_vid,
            "tempo_vid": tempo_vid,
        })
        ...
        for track_spec in spec.tracks:
            await self.create_track(name=track_spec.config.name, tid=track_spec.track_id)
            tvid = await self.commit_track_version(
                tid=track_spec.track_id,
                spec=track_spec,
                schema_version=schema_version,
            )
            await self._execute_void(CQL_LINK_COMPOSITION_TRACK, {
                "cvid": cvid,
                "tvid": tvid,
            })
