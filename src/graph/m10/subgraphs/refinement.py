"""
Refinement Subgraph — M10: InstrumentedGraph with interceptors.

KEY CONCEPT: Uses InstrumentedGraph for observability (logging, metrics).
Nodes use the M9 pattern (registry handler direct calls). No StateMediator.
...
def _scope_classifier(state: RefinementState) -> dict:
    ...
    Returns a plain dict — passes through mediator unchanged.
    """
...
def _make_plan_refiner(registry: ToolRegistry):
    """Factory: plan_refiner node that calls refine_plan via registry."""
...
def build_refinement_subgraph(
    ...
    """Build the refinement subgraph with InstrumentedGraph.
    ...
    builder.add_node("plan_refiner", _make_plan_refiner(registry))
