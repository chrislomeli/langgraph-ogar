"""
Refinement Subgraph -- M11: PlanOrchestrator-driven refinement.

KEY CONCEPT: Refinement uses the PlanOrchestrator's refine() method to
re-plan targeted scopes and automatically invalidate downstream sub-plans.

The orchestrator:
  1. Loads the existing PlanGraph from state (or rebuilds it)
  2. Applies refinement to plan_composition (re-plan with user prompt)
  3. Automatically invalidates compile_composition (downstream)
  4. Re-executes invalidated steps
"""
...
from framework.langgraph_ext.planning.models import PlanGraph, SubPlan, SubPlanStatus, RefinementRequest
...
def _make_orchestrated_refine(scope_registry: ScopeRegistry):
    """Factory: refinement node that uses PlanOrchestrator."""
    ...
    def orchestrated_refine(state: RefinementState) -> dict:
        ...
        # Rebuild a PlanGraph from existing artifacts, marking steps as done
        plan_graph = PlanGraph(
            ...
            sub_plans={
                "sketch_parse": SubPlan(
                    ...
                    status=SubPlanStatus.done,
                    result=None,  # sketch not needed for refinement
                ),
                "plan_composition": SubPlan(
                    scope_id="plan_composition",
                    scope_type="plan_composition",
                    content={"plan_bundle": previous_plan},
                    status=SubPlanStatus.done,
                ...
                "compile_composition": SubPlan(
                    ...
                    content={},
                    status=SubPlanStatus.done,
                    result=previous_compile_result,
                ),
            },
            dependencies={
                "sketch_parse": set(),
        ...
        # Apply refinement directly instead of through orchestrator.refine()
        # (since our refinement needs the user prompt, not just re-planning)
        user_prompt = state["user_message"]
        refined_plan = _planner.refine(previous_plan, user_prompt)

        # Update the plan_composition sub-plan with refined result
        plan_sp = plan_graph.get("plan_composition")
        plan_sp.content = {"plan_bundle": refined_plan, "needs_refine": False}
        plan_sp.result = refined_plan
        plan_sp.status = SubPlanStatus.done
        ...
        # Invalidate and re-execute compile_composition
        compile_sp = plan_graph.get("compile_composition")
        compile_sp.status = SubPlanStatus.approved  # ready to re-execute

        if previous_compile_result and changed_ids != all_voice_ids:
            compile_sp.content = {
                "regenerate_voices": changed_ids,
                "previous_compile_result": previous_compile_result,
            }

        # Run orchestrator to execute the remaining step
...
def build_refinement_subgraph(scope_registry: ScopeRegistry | None = None):
    """Build the refinement subgraph with PlanOrchestrator.
    ...
    builder.add_node("orchestrated_refine", _make_orchestrated_refine(scope_registry))
